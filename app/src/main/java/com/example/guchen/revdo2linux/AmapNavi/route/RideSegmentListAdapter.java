...
public class RideSegmentListAdapter extends BaseAdapter {
	...
	public RideSegmentListAdapter(Context applicationContext,
		...
		mItemList.add(new RideStep());	//RideStep是骑行API的一个类，此处添加出发头position=0
		for (RideStep rideStep : steps) {//在list中添加要显示的路径信息
			mItemList.add(rideStep);
		}
		mItemList.add(new RideStep());  //此处添加到达尾的list
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (position == 0) { //出发头item
			holder.dirIcon.setImageResource(R.drawable.start);
			...
			return convertView;
		} else if (position == mItemList.size() - 1) { //达到尾item
